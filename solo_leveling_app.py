...
# --- PENALTY BOX SECTION (with Debt System) ---
st.header("System Penalties")
with st.expander("Apply penalties for distractions"):
    st.write("Check your phone's screen time and apply penalties honestly, Hunter!")
    
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("Penalty: Insta > 30 mins (-15 G)", use_container_width=True):
            hunter['gold'] -= 15
            st.warning(f"Penalty Applied! -15 Gold for Instagram. Your new balance is {hunter['gold']} G.")
            save_data()
            st.rerun()

    with col2:
        if st.button("Penalty: YouTube > 1 hour (-25 G)", use_container_width=True):
            hunter['gold'] -= 25
            st.warning(f"Penalty Applied! -25 Gold for YouTube. Your new balance is {hunter['gold']} G.")
            save_data()
            st.rerun()
